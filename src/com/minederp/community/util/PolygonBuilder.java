package com.minederp.community.util;
...
import com.minederp.community.games.communities.content.PolygonChecker;
...
public class PolygonBuilder {
	...
	private void drawPolygon(Material mat, Material mat2, World world) {
		...
		for (int i = 0; i < polygon.size(); i++) {
			...
			gameLogic.addBlockForReprint(new BlockReprintItem(p.X, p.Y, p.Z, bl.getTypeId(), bl.getData(), "Line" + guid));
			bl.setType(mat2);
		}
	...
	public void drawLineInternal(World world, Material type, int p1X, int p1Y, int p2X, int p2Y) {
		...
		while (true) {
			...
			gameLogic.addBlockForReprint(new BlockReprintItem(p1X, Y, p1Y, bl.getTypeId(), bl.getData(), "Line" + guid));
			bl.setType(type);
