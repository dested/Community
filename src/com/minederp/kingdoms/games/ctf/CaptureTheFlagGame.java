...
import com.minederp.kingdoms.games.BlockReprintItem;
...
public class CaptureTheFlagGame extends Game {
	...
	public CaptureTheFlagGame(CommunityPlugin kingdomsPlugin) {
		...
		if (drawingRectangle == 2 && actingPlayer != null && actingPlayer.getName().equals(movingPlayer.getName())) {
			...
			if (mRectangle.x > mRectangle.x + mRectangle.width)
				for (int x = mRectangle.x; x >= mRectangle.x + mRectangle.width; x--) {
					...
					this.logic.addBlockForReprint(new BlockReprintItem(x, y, mRectangle.y, (bc = movingPlayer.getWorld().getBlockAt(x, y, mRectangle.y))
							.getTypeId(), bc.getData(), "Drawing"));
					bc.setType(Material.DIAMOND_BLOCK);
					this.logic.addBlockForReprint(new BlockReprintItem(x, y, mRectangle.y + mRectangle.height, (bc = movingPlayer.getWorld().getBlockAt(x,
			...
			if (mRectangle.y > mRectangle.y + mRectangle.height)
				for (int z = mRectangle.y; z >= mRectangle.y + mRectangle.height; z--) {
					...
					this.logic.addBlockForReprint(new BlockReprintItem(mRectangle.x, y, z, (bc = movingPlayer.getWorld().getBlockAt(mRectangle.x, y, z))
							.getTypeId(), bc.getData(), "Drawing"));
					bc.setType(Material.DIAMOND_BLOCK);
					this.logic.addBlockForReprint(new BlockReprintItem(mRectangle.x + mRectangle.width, y, z, (bc = movingPlayer.getWorld().getBlockAt(
